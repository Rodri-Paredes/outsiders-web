...
import { Loader2, Search, Trash2, ArrowUp, ArrowDown, Sparkles } from 'lucide-react'

interface NewInConfig {
  title: string
  product_ids: string[]
}
...
const DEFAULT_CONFIG: NewInConfig = {
  title: 'New In',
  product_ids: [],
}

export default function NewInEditor() {
  const [config, setConfig] = useState<NewInConfig>(DEFAULT_CONFIG)
  ...
  const loadConfig = async () => {
    try {
      setLoading(true)
      const data: NewInConfig = await cmsService.getConfig('new_in')
      ...
      toast.error('Error al cargar New In')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const newConfig: NewInConfig = {
        title: config.title,
        product_ids: selectedProducts.map(p => p.id),
      }
      await cmsService.setConfig('new_in', newConfig)
      setConfig(newConfig)
      toast.success('New In guardado ✓')
    } catch (err: any) {
    ...
    <div className="max-w-5xl space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">
          Selecciona y ordena los productos que aparecen en la sección <strong>New In</strong> del home. Si no hay productos seleccionados, se muestran automáticamente los más recientes.
        </p>
          ...
          <div className="min-h-[300px] bg-gray-50 rounded-xl border border-gray-100 p-3 space-y-2">
            {selectedProducts.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center py-12 text-gray-400">
                ...
                <p className="text-xs mt-1 text-gray-300">Si dejas vacío, se muestran los más recientes automáticamente</p>
              </div>
            ) : (
